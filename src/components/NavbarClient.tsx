...
export default function Navbar({ initialProfile }: { initialProfile: any }) {
  const { profile: authProfile, loading, signOut } = useAuth();
  const router = useRouter();

  // Use server-side profile if available for the first paint
  const profile = authProfile || initialProfile;
  const isInitialLoad = loading && !initialProfile;
